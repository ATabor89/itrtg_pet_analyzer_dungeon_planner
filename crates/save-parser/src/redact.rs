...
mod tests {
    ...
    fn redacts_known_root_identity_fields() {
        let plaintext = "s:RedactedAccount;W:RedactedGod;c:1781053129;\
                         001:00000000000000000;002:RedactedGod;\
                         003:a_0000000000000000000;004:Redacted Name;005:123;";
        ...
        assert!(changes.iter().any(|c| c.key == "001" && c.old == "00000000000000000"));

        let out = root.serialize();
        // Every redacted value is gone, including the partial "Redacted".
        let olds: Vec<&str> = changes.iter().map(|c| c.old.as_str()).collect();
        assert!(residual_hits(&out, &olds).is_empty());
        assert!(!out.contains("Redacted"));
        assert!(!out.contains("00000000000000000"));
    ...
    fn redaction_is_idempotent_and_skips_absent_fields() {
        ...
        let mut root = raw::parse("s:RedactedAccount;c:1;");
