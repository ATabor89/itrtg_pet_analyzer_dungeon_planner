...
fn seed() -> Vec<FieldDef> {
    ...
    let mut v = vec![
        ...
        def(&["a"], "Total Divinity", Number, Resources, "Running divinity balance, large double (root a)"),
        def(&["K", "g"], "Divinity Capacity In Use", Number, Resources, "Divinity currently held in the generator, not the total (K.g)"),
        def(&["K", "c"], "Divinity Gen Worker Clones", Number, Resources, "Worker Clones allocated to the Divinity Generator (K.c)"),
        def(&["K", "n"], "Divinity Stone Storage", Number, Resources, "Stones held in the generator's Stone Storage (K.n)"),
        ...
        def(&["T"], "Planet (Ultimate Beings)", Text, RawTree, "root.T = AIDFNOPNJGK, marker Planet"),
