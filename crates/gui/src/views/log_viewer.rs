...
fn compute_party_order(pets: &[log_parser::PetInfo], dungeon_name: &str, dungeon_level: &str) -> Vec<usize> {
    let yaml = crate::platform::load_dungeon_recommendations().unwrap_or_default();
    log_parser::compute_party_order(pets, &yaml, dungeon_name, dungeon_level)
}
