...
| `V` | **mights** (`DBLLJGOKFJG`; **15**, enum `DIPOABKGODI` 0..=14 — `items::might_name`; **id 14 = Black Hole +**, from the 1K-Clones-Black-Hole challenge) | `b` = **current level** (specials all 64 = the user's "at level 64" ✓; Σ levels = 3,200 = the in-game White Tiger unlock progress ✓), `m` = target level, `n` = clone-spread ratio. **Usable-skill specials (ids 8–13): `e` = True (is usable), `g` = base unleash duration (+1 s/level), `h` = cooldown ms remaining (0 = ready), `l` = active-buff timer ms (>0 while live)**, `i`/`j`/`k` = HP-recovery/Attack/Mystic % (`e`/`h`/`l` confirmed 2026-06-23 via the Crystals save). Editor: a "Reset all cooldowns" button zeroes `h`+`l` on every usable Might. "Total Might" (100,983) is the all-rebirths global at `x.129`/`p.F` |
...
| `q` | **Bonus Points** (`MLILKGIALMB.PIDMNDGHPPM`, BigDouble) — the currency shown on the **Daily screen** ("You currently have N Bonus points") but stored here on the pet object, not on the daily `p` block. Fixture = 1. Wired into the Daily section. | C# `HBLDMFPGCEJ:3948` reads `…FIHAENJIDAO.PIDMNDGHPPM`; deser key `q` at `MLILKGIALMB:10066` |
...
| `w` | **Crystal Factory** (single struct, `GKJLJMJLMIB`, marker "CrystalFactory") — **corrected 2026-06-23** (Crystals save + notes) | `a` = Crystal Power (400000), `b` = the 6 per-type **modules** (`IPNJFMOMAMB`: `a` = type 0 Physical/1 Mystic/2 Battle/3 Creation/4 Ultimate/5 God [`KEMALIHPLCG`], `b` = level [a module makes `level` grade-1 crystals per cycle], **`e` = the module's stored grade-1 crystal stack** [`CNJMKHCJPGM` `{a:type, b:grade=1, c:count}` — confirmed: Battle `e.c`=1814, Ultimate 297, God 1279], `c` = upgrade cost, `d` = cycle timer, `f` = value), **`c` = Energy** (spent to upgrade modules, +per-UB-defeat; confirmed = 2058), **`e` = equipped crystals** (`CNJMKHCJPGM` `{a:type, b:grade, c:0}` — confirmed: Battle grade 23, Ultimate 16, God 12), `d` = BigDouble progress timer. (Earlier "clones / accumulated" labels were wrong — the `e` stacks are crystals, `c` is Energy.) Wired into the editor (Crystal Factory section: Crystal Power + Energy + module level + stored grade-1 count editable, equipped-crystal grades editable). |
...
| `F` | **partner pet type id** (999 = none) | 158/158 match export Partner column; pairs are mutual (Cat↔Dog, Vampire↔Succubus, Gnome↔Dwarf), a pet may self-partner (own type id; e.g. Nugget 141→141). Wired into the editor's **Partners** section (mutual set/clear with valid-only picker + "Assign all" auto-pairing; odd one out self-partners) |
