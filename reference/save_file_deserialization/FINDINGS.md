...
| `i` | **creations** (29, Shadow Clone … Universe — `items::creation_name`) | `d` = current amount (Shadow Clone entry = the clone count ✓, Earthlike Planet 0), `e` = clone cost (Light 1,000 … Earthlike Planet 198,774), `i` = next-at ✓, `g` = unidentified cumulative (grew +16M for Light in a day but only 3,893 for Earthlike Planet — *not* total-created in the Earth-Eater sense), `f/h/j` = flags (auto-create settings?) |
