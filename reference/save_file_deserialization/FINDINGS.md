...
| `h` | **Physical training skills** (28 — `items::physical_skill_name`, list order = the in-game Physical screen: Running … Train on Dimension X) | `a` = id/list position, `b` = **level**, `c` = **clones allocated**, `d` = 0 (unidentified). Player-confirmed 2026-06-18 by removing clones from some Physical skills (leaving Mystic synced) and watching `b`/`c` diverge as expected |
...
| `j` | **Mystic training skills** (28 — `items::mystic_skill_name`: Double Punch … Time Manipulation) | same shape as `h` (`a`/`b`/`c`/`d`), plus a Mystic-only `e` sub-struct (`e.a` index, `e.b` slow-drifting, `e.c` small stable int) — **left unidentified** (player theory: training caps). NB the byte-identical `b` between Physical[i] and Mystic[i] in this fully-reduced Steam save is the in-game **"Sync"** toggle (keeps clone counts/levels equal), not a shared value |
...
| `d`, `h` | instance id + mirror. Usually equal, **but not always**: on a 2026-06-17 real save many entries have `d = 0` while `h` is unique and increasing with list order (the first items had `d=0`/`h=2,3,7,…`; a Magic Stick `d=0`/`h=858`). So **`h` (mirror) is the reliable unique id**; a pet slot of `0` means *empty*. Cause of the `d=0` entries not yet understood (some may be checked out to a smith); the game tolerates it. The editor resolves slots by `h` first, then `d`, and treats `0` as empty. | referenced by pet `w.e/f/g` and Pet Equips export |
