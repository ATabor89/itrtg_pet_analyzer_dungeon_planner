...
| `T` | **Planet system** (`AIDFNOPNJGK`, marker **"Planet"**) — **corrected** (was mislabeled "Baal Slayer block"; marker + contents are definitively the Planet / Ultimate-Beings system; the 4 in-game planet screens likely map to its sub-blocks) | **`f` = the 5 Ultimate Beings** (`CEFAAPALBMD`, screen-1 "UBs"): each `c` = UB id (1 Planet Eater / 2 Godly Tribunal / 3 Living Sun / 4 God Above All / 5 ITRTG; spawn intervals 1–5 h), `b` = kill count, **`d` = spawn countdown ms** (counts DOWN; spawns at ≤0 — set 0 to force a spawn), `e` = alive, `f` = GP gained. Wired into the editor (titled by UB). **`k` = 5 Powersurge-like tracks** (`FPBMNCNKPHN`: `a`≈multiplier ~100, `c`=1–5 per-UB, `e`/`i`=levels — likely the per-UB Powersurges + clone allocation; needs in-game anchoring). Scalars (first-pass, need anchors): `a` bool, `b`/`c`/`e`/`g`/`j`/`l` BigDouble (planet power / multiplier / accumulated power — `b` used as hours via `/3600000`), `d` BigDouble (planet level?), `h`/`i` int (`i`=100, likely the **Planet Multiplier** base). **NOTE:** overturns the prior "T.h = Baal Power / T.k = TBS tracks" labels — those were misattributions; where Baal Power / TBS actually live is now an open question. |
